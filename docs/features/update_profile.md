...
```mermaid
flowchart TD
    A(["Aplikasi Mulai"]) --> B["Inisialisasi Plugin & Izin OS"]
    B --> C(["Standby"])
    
    C --> D["User Simpan Profil"]
    D --> E{"Status API"}
    ...
    classDef main fill:#c8e6c9,stroke:#388e3c,color:#000
    classDef process fill:#e1bee7,stroke:#8e24aa,color:#000
